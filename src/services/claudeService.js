// FileName: src/services/claudeService.js (REVISED VERSION)
...
const CONFIG = {
  DELAYS: {
    BETWEEN_CALLS: 1200,      // Increased for better rate limiting
    RETRY_DELAY: 2500,        // Increased for better error recovery
    PROCESSING_DELAY: 500     // For UI updates
  },
  TOKENS: {
    DEFAULT_MAX: 4096,
    EXTENDED_MAX: 8192,       // Claude Sonnet 4 supports up to 8K output
    CHUNK_MAX: 3500,          // Conservative chunking
    INPUT_LIMIT: 150000       // Approximate input token limit
  },
  API: {
    MAX_RETRIES: 3,
    TIMEOUT: 45000,           // Increased timeout for complex content
    RATE_LIMIT_DELAY: 5000    // Delay when rate limited
  },
  VALIDATION: {
...
// =====================================================
// SUBJECT CATEGORIZATION & MAPPINGS
// =====================================================
  ...
  static sanitizeContent(content) {
    ...
    return content
      ...
      .replace(/[^\x00-\x7F]/g, (char) => {
        // Keep common educational characters, replace others
...
class ContentAnalyzer {
  static analyzeStructure(content) {
    ...
    analysis.contentType = this.determineContentType(analysis, content);
    analysis.estimatedReadingLevel = this.estimateReadingLevel(analysis);
  ...
  static calculateComplexity(analysis) {
    ...
    return {
      ...
      processingTime: Math.max(30, Math.min(120, complexityScore * 2)) // estimated seconds
    };
  }
  
  static determineContentType(analysis, content) {
  ...
  static estimateReadingLevel(analysis) {
...
// =====================================================
// WIDA STANDARDS & OBJECTIVES ENGINE
// =====================================================

class WIDAAlignmentEngine {
  static extractContentObjectives(originalContent, subject, contentAnalysis) {
    const objectives = [];
    
    // Look for existing objectives first
    ...
    // Generate subject-specific objectives if none found
    if (objectives.length === 0) {
      const subjectGroup = this.getSubjectGroup(subject);
      objectives.push(...this.generateSubjectObjectives(subject, subjectGroup, contentAnalysis));
    }
    
    return objectives.slice(0, 4); // Limit to 4 main objectives
  }
  
  static getSubjectGroup(subject) {
    ...
    return SUBJECT_GROUPS.APPLIED; // Default fallback
  }
  
  static generateSubjectObjectives(subject, subjectGroup, contentAnalysis) {
    const objectives = [];
  ...
  static createLanguageObjectives(contentObjectives, proficiencyLevel, contentAnalysis, subject) {
    const level = WIDA_LEVELS[proficiencyLevel.toLowerCase()]?.level || 3;
    const subjectGroup = this.getSubjectGroup(subject);
    const primaryKLU = this.determinePrimaryKLU(contentAnalysis, subjectGroup);
    
    const objectives = [];
    
    // KLU-specific objectives
    objectives.push(this.getKLUObjective(primaryKLU, level));
    
    // Level-specific language objectives
    switch (level) {
      case 1: // Entering
  ...
  static determinePrimaryKLU(contentAnalysis, subjectGroup) {
    // Analyze content to determine Key Language Use
    if (contentAnalysis.hasReadingPassage && contentAnalysis.openEndedQuestions > 3) {
      return 'Argue'; // Analytical reading requires argumentation
    } else if (contentAnalysis.hasMath || subjectGroup.name === 'STEM') {
      return 'Explain'; // STEM subjects primarily explain processes
    } else if (contentAnalysis.dates > 0 || contentAnalysis.hasDialogue) {
      return 'Narrate'; // Sequential or story-based content
    } else {
      return 'Inform'; // Default for descriptive content
    }
  }
  
  static getKLUObjective(klu, level) {
    const objectives = {
      'Narrate': [
        'sequence events using time words and visual supports', // Level 1
        'recount events using simple past tense and sequence words', // Level 2
        'tell detailed stories with clear beginning, middle, and end', // Level 3
        'produce complex narratives with dialogue and descriptive language', // Level 4+
        'craft sophisticated narratives using varied literary devices'
      ],
      'Explain': [
        'show how or why using drawings, gestures, and single words', // Level 1
        'describe processes using simple sentences with "because"', // Level 2
        'explain cause-and-effect relationships using transition words', // Level 3
        'interpret complex processes using technical vocabulary', // Level 4+
        'evaluate multiple explanations using precise academic language'
      ],
      'Argue': [
        'express preferences using yes/no responses and choice cards', // Level 1
        'state simple opinions using sentence frames', // Level 2
        'support opinions with reasons using compound sentences', // Level 3
        'construct arguments with claims, evidence, and counterarguments', // Level 4+
        'debate complex issues using sophisticated rhetorical strategies'
      ],
      'Inform': [
        'identify and label information using word banks and visuals', // Level 1
        'describe information using adjectives and simple sentences', // Level 2
        'compare and contrast information using specific vocabulary', // Level 3
        'synthesize information from multiple sources using cohesive devices', // Level 4+
  ...
  static determineWIDAStandards(subject) {
...
class ProficiencyAdapter {
  static getAdaptationRules(proficiencyLevel) {
    const level = WIDA_LEVELS[proficiencyLevel.toLowerCase()]?.level || 3;
...
class APIManager {
  static async callClaude(messages, maxTokens = CONFIG.TOKENS.DEFAULT_MAX) {
    ...
    for (let attempt = 1; attempt <= CONFIG.API.MAX_RETRIES; attempt++) {
      try {
        ...
        const sanitizedMessages = messages.map(msg => ({
          role: msg.role,
          content: ContentValidator.sanitizeContent(msg.content)
        }));
        ...
        if (!response.ok) {
          ...
          if (response.status === 429) {
            if (attempt < CONFIG.API.MAX_RETRIES) {
              ...
              await this.delay(CONFIG.API.RATE_LIMIT_DELAY);
              continue;
            }
          }
        ...
        await this.delay(CONFIG.DELAYS.RETRY_DELAY * attempt);
      }
    }
  }
...
class ContentGenerator {
  static async generateWorksheet(params, contentAnalysis, adaptationRules) {
    const { subject, proficiencyLevel, materialType } = params;
    
    const prompt = this.buildWorksheetPrompt(params, contentAnalysis, adaptationRules);
    ...
    const result = await APIManager.callClaude([{ role: 'user', content: prompt }], tokenLimit);
    return result.content[0].text;
  }
  
  static buildWorksheetPrompt(params, contentAnalysis, adaptationRules) {
    const {
      subject,
      proficiencyLevel,
      materialType,
...
Students will be able to: ${learningObjectives}`;
    ...
    // Add bilingual support instructions
...
Generate the complete worksheet with all original content preserved and adapted to ${proficiencyLevel} level. Ensure it's ready to print and use immediately.`;
  ...
  static async generateTeacherGuide(studentWorksheet, params, contentAnalysis) {
    const { subject, proficiencyLevel } = params;
    const adaptationRules = ProficiencyAdapter.getAdaptationRules(proficiencyLevel);
    
    // Extract content and language objectives
    const contentObjectives = WIDAAlignmentEngine.extractContentObjectives(
      params.contentToAdapt, subject, contentAnalysis
    );
    
    const languageObjectives = WIDAAlignmentEngine.createLanguageObjectives(
      contentObjectives, proficiencyLevel, contentAnalysis, subject
    );
    
    const widaStandards = WIDAAlignmentEngine.determineWIDAStandards(subject);
    const keyVocabulary = this.extractKeyVocabulary(studentWorksheet);
...
- **Main Activity:** ${this.calculateActivityTime(contentAnalysis)} minutes
- **Review & Discussion:** 10-15 minutes
- **Total Estimated Time:** ${this.calculateTotalTime(contentAnalysis)} minutes
...
${params.includeBilingualSupport && params.listCognates ? this.generateCognateSection(params.nativeLanguage, studentWorksheet) : ''}
...
Focus on content understanding while allowing for developing language proficiency. Accept language that demonstrates comprehension even if not grammatically perfect.`;
  ...
  static extractKeyVocabulary(worksheet) {
    const boldTerms = (worksheet.match(/\*\*(.*?)\*\*/g) || [])
      .map(term => term.replace(/\*\*/g, ''))
      .slice(0, 10); // First 10 terms
  ...
  static calculateActivityTime(contentAnalysis) {
    const baseTime = 15;
  ...
  static calculateTotalTime(contentAnalysis) {
    const activityTime = this.calculateActivityTime(contentAnalysis);
    return activityTime + 25; // Add time for vocab and review
  }
  
  static generateCognateSection(nativeLanguage, worksheet) {
...
class AdaptationOrchestrator {
  static async processLongContent(params, setProcessingStep, contentAnalysis) {
    // Two-step strategy for lengthy reading comprehension content
    setProcessingStep?.('Step 1: Generating worksheet structure...');
    
    const adaptationRules = ProficiencyAdapter.getAdaptationRules(params.proficiencyLevel);
    
    // Step 1: Create structure with placeholder
...
Create the complete structure but use {{READING_PASSAGE_PLACEHOLDER}} for the actual passage.`;
    ...
    const structureResult = await APIManager.callClaude([{ role: 'user', content: structurePrompt }]);
    
    // Step 2: Insert the actual content
    setProcessingStep?.('Step 2: Inserting complete reading passage...');
    
    const originalPassage = this.extractReadingPassage(params.contentToAdapt);
  ...
  static extractReadingPassage(content) {
    // Attempt to extract the main reading passage from content
    ...
    for (let line of lines) {
      ...
      // Skip obvious non-passage content
...
/**
 * Main adaptation function - works with any content type and subject
 */
export const adaptMaterialWithClaude = async (params, setProcessingStep) => {
  ...
  try {
    ...
    const validation = ContentValidator.validateInput(params);
    ...
    const contentAnalysis = ContentAnalyzer.analyzeStructure(params.contentToAdapt);
    ...
    if (isLongContent && contentAnalysis.contentType === 'reading_comprehension') {
      ...
      studentWorksheet = await AdaptationOrchestrator.processLongContent(params, setProcessingStep, contentAnalysis);
    } else {
      console.log('Using standard generation strategy');
      setProcessingStep?.('Generating adapted student worksheet...');
      studentWorksheet = await ContentGenerator.generateWorksheet(params, contentAnalysis, adaptationRules);
    }
    
    // Step 4: Quality validation and fixes
    setProcessingStep?.('Validating worksheet quality...');
    let validation = QualityValidator.validateWorksheet(studentWorksheet);
    
    if (!validation.isValid) {
      console.warn('Quality issues detected:', validation.issues);
      studentWorksheet = QualityValidator.fixCommonIssues(studentWorksheet);
      validation = QualityValidator.validateWorksheet(studentWorksheet);
    }
    
    // Step 5: Generate teacher guide
    await APIManager.delay(CONFIG.DELAYS.BETWEEN_CALLS);
    setProcessingStep?.('Generating comprehensive teacher guide...');
    
    const teacherGuide = await ContentGenerator.generateTeacherGuide(
      studentWorksheet, params, contentAnalysis
    );
    
    // Step 6: Create WIDA-aligned descriptors
    ...
    const dynamicWidaDescriptors = {
      ...
      contentObjectives,
      languageObjectives,
      widaStandards: WIDAAlignmentEngine.determineWIDAStandards(params.subject),
      descriptors: [
    ...
    return {
      ...
      imagePrompts: null, // Could be enhanced in future
      metadata: {
        contentAnalysis,
        adaptationRules,
        validation,
